USE [pv42]
GO
/****** Object:  UserDefinedFunction [dbo].[problemdetail2]    Script Date: 3/31/2017 10:41:56 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER FUNCTION [dbo].[problemdetail2]
(	
...
from @normal n join bniviewbasedata b on (n.deviceid = b.id)
join devicehierarchy d on (n.deviceid=d.deviceid) where hierlongname = @hier
UNION
Select n.deviceid,null, lokasi, wilayah, cabang, vendor, flm 
from @sleep n join bniviewbasedata b on (n.deviceid = b.id)
join devicehierarchy d on (n.deviceid=d.deviceid) where hierlongname = @hier
